...
public final class TagUtil {
    ...
    private static String normalize(final String title) {
        ...
        Collections.sort(iconTags, (t1, t2) -> {
            ...
            final String u2Title = t2.optString(TagUtil.TAG_T_TITLE_LOWER_CASE);

            return u2Title.length() - u1Title.length();
        });

        for (final JSONObject iconTag : iconTags) {
            final String iconTagTitle = iconTag.optString(TagUtil.TAG_TITLE);
        ...
        for (final JSONObject tag : allTags) {
            final String tagURI = tag.optString(TagUtil.TAG_URI);
            final String tagTitle = tag.optString(TagUtil.TAG_TITLE);
            if (tagURI.equals(tagTitle)) {
                continue;
            }

            if (StringUtils.equals(title, tagURI)) {
                return tag.optString(TagUtil.TAG_TITLE);
            }
        }
