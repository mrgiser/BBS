...
public class ArticleCache {
    ...
    public void loadSideHotArticles() {
        ...
        final ArticleMapper articleMapper = beanManager.getReference(ArticleMapper.class);
        ...
        try {
            ...
            LOGGER.error( "Loads side hot articles failed", e);
        } finally {
            Stopwatchs.end();
        }
    }
    ...
    public void loadSideRandomArticles() {
        ...
        try {
            final List<JSONObject> articles = articleMapper.getRandomly(Symphonys.getInt("sideRandomArticlesCnt") * 5);
            articleQueryService.organizeArticles(UserExt.USER_AVATAR_VIEW_MODE_C_STATIC, articles);

            SIDE_RANDOM_ARTICLES.clear();
            SIDE_RANDOM_ARTICLES.addAll(articles);
        } catch (final MapperException e) {
            LOGGER.error( "Loads side random articles failed", e);
        } finally {
            Stopwatchs.end();
        }
    }
    ...
    public void loadPerfectArticles() {
        ...
        try {
            ...
            LOGGER.error( "Loads perfect articles failed", e);
        } finally {
            Stopwatchs.end();
        }
    }
