...
public class AudioMgmtService {
     ...
     * Logger.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(AudioMgmtService.class);
    ...
    private static void refreshBaiduAccessToken() {
        ...
        try {
            ...
            LOGGER.error( "Requires Baidu Yuyin access token failed", e);
        }
    }
    ...
    public String tts(final String text, final String type, final String textId, final String uid) {
        ...
        try {
            ...
            if (e instanceof QiniuException) {
                try {
                    LOGGER.error( "Uploads audio failed [type=" + type + ", textId=" + textId + "], Qiniu exception body [" +
                            ((QiniuException) e).response.bodyString() + "]");
                } catch (final Exception qe) {
                    LOGGER.error( "Uploads audio and parse result exception", qe);
                }
            } else {
                LOGGER.error( "Uploads audio failed [type=" + type + ", textId=" + textId + "]", e);
            }
        }
