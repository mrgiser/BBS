...
public final class Gravatars {
     ...
     * Logger.
     */
    public static final Logger logger = LoggerFactory.getLogger(Gravatars.class);
    ...
    public static byte[] getRandomAvatarData(final String hash) {
        try {
            ...
            if (HttpServletResponse.SC_OK != response.statusCode()) {
                logger.warn( "Gets avatar data failed [sc=" + response.statusCode() + "]");
            ...
            logger.error(  "Gets avatar data failed", e);
